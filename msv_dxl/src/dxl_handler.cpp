...
#include "../include/msv_dxl/dxl_handler.h"
...
DxlHandler::DxlHandler (const char *portname) {
  port_name = (char*)portname;
  socket_fd = -1;
  setBaudRate(115200);
}

bool DxlHandler::openPort () {
...
void DxlHandler::closePort () {
  if(socket_fd != -1)
    close(socket_fd);
  socket_fd = -1;
}

void DxlHandler::clearPort () {
  tcflush(socket_fd, TCIFLUSH);
}

void DxlHandler::setPortName (const char* portname) {
  port_name = (char*)portname;
}

char* DxlHandler::getPortName () {
  return port_name;
}

int DxlHandler::setBaudRate (const int baudrate) {
...
int DxlHandler::getBaudRate ()
{
  return remapBaudRate(baudrate_);
}

int DxlHandler::getBytesAvailable () {
  int bytes_available;
  ioctl(socket_fd, FIONREAD, &bytes_available);
  return bytes_available;
}

int DxlHandler::readPort (uint8_t *packet, int length) {
  return read(socket_fd, packet, length);
}

int DxlHandler::writePort (uint8_t *packet, int length) {
  return write(socket_fd, packet, length);
}

int DxlHandler::setInterfaceAttribs (int fd, int baudrate, int parity) {
  struct termios tty;
...
void DxlHandler::setBlocking (int fd, int block) {
  struct termios tty;
  memset(&tty, 0, sizeof tty);
...
int DxlHandler::remapBaudRate (const int baudrate) {
  switch (baudrate) {
    case B9600:
      return 9600;
    case B19200:
